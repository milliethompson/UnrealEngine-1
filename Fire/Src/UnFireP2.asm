;///////////////////////////////////////////////////////////////////////////
;//
;//  UnFireP2.asm  -  self-modifying code optimized for PPro/PII processors.
;//
;//  version 1.1   December 1997
;//
;//  This is a part of the Unreal Texture Animation Engine code.
;//  Copyright (C) 1997,1998 by  Epic Megagames and Evolution Software.
;//
;////////////////////////////////////////////////////////////////////////////
;
;------------------------------------------------------------------
...
;
; Filename: UnFireP2.asm
;
; default options : ML /c /Cx /coff  UnFireP2.asm
;
; Assemble/build syntax  in Microsoft Developer Studio:
;
;   Build->Settings->[select UnFireP2] ->Custom build:
;
;   ML /c /Cx /coff  /Fo$(OutDir)\UnFireP2.obj UnFireP2.asm
;
...
;in .DATA :
;
; SelfModFireP2        fire update code
; SelfModFireWrapP2    fire update code with wraparound
;
;in .CODE :
;
; CalculateFireP2      entry for fire algorithm, calls SelfModFire
; CalcWrapFireP2       entry for fire algorithm, calls SelfModFireWrap
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;


.586                 ; 486 or 586 don't differ in MASM output.
...
;============================
 ...
 WIntoCoreX:
  ...
  MOV [EBP+2-4],DL       ;; Store pixel 3 of 4
  ...
  MOV DL,Advb[EBX+ECX]   ;;no AGI if pairing as expected
         org $-4
         WTable_Patch4 DD 0
  ;---------------------------- making pixel 0,1
  MOV BL,Advb[EBP]          ;; CellA
  ...
  MOV [EBP+3-4],DL       ;; Store pixel 4 of 4
...
WEnterCoreX:
  ...
  MOV DL,Advb[EBX+ESI] ;; no AGI if pairing as expected
         org $-4
         WTable_Patch2   DD 0
  ;---------------------------- making pixel 2,3
  MOV   BL,Advb[EBP]
  ...
  MOV   [EBP+1],DL  ;; Store pixel 2 of 4
  ADD   EBP,4       ;;
...
;  Need wrap-aroundignore AL/ CELL3...; AL has the wrong one !
  ...
  MOV   [EBP+2-4],DL  ;; sotre pixel 3 of 4
...
SelfModFireSlowWrapP2 PROC
   ...
   ;; Patch cell line 1
   MOV EAX,0   ;;LocalLineXSize

   call SPatchCellLine1
...
;============================
SPatchCellLine1 label near

   MOV ECX,EAX ;save

   MOV SWCell1_Patch0,EAX

   INC EAX
   MOV SWCell2_Patch0,EAX
   MOV SWCell2_Patch2,EAX

   MOV EBX,EAX
   ADD EBX,2
   MOV SWCell2_Patch1_p2,EBX

   INC EAX
   MOV SWCell3_Patch1,EAX

   SUB EAX,2
   MOV SWCell3_Patch2_n2,EAX
   ...
   ADD EAX,2 -2 ;; end of row, - Xsize (+2 for addressing cell3)
                ;; -2 for EBP bias.. ==0 !
   MOV SWCell3_SlowWrapPatch0_n2,EAX ;; put in (-X-?) := as for cell2 +1
   ;;;;;;;;;
...
;;========= EBP got advanced +LineLen ; check for end of whole bitmap now.
  ...
  MOV   [EBP+3-4],DL  ;; sotre pixel 4 of 4
  JB    SWIntoCoreY
...
CalcWrapFireP2 PROC PBitmap:DWORD, \
				     Ptable:DWORD, \
					   XVar:DWORD, \
					   YVar:DWORD
...
CalcSlowFireP2 PROC PBitmap:DWORD, \
                     Ptable:DWORD, \
                       XVar:DWORD, \
                       YVar:DWORD

    ;;
    ;; As CalcWrapFireP2; but the destination-line is also the source-line.
    ;;
